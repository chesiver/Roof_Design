...
//************************************************************************
//**** Function For Alpha-Shape
//************************************************************************
  //ui
  void moveWithMouse(pt P){P.x += mouseX - pmouseX; P.y += mouseY - pmouseY;}
  
  //display
  void arrow(pt P, pt Q) {arrow(P, V(P,Q)); } 
  void arrow(pt P, vec V) 
  {
    ...
    // otherwise continue
       float s=max(min(0.2, 20.0 / n), 6.0 / n);       // show V as arrow from P 
       pt Q = P(P, V); 
       vec U = V(-s, V); 
       vec W = R(V(0.3, U)); 
       beginShape(); 
  ...
  //compute
  boolean isInDiskOnRight(pt Q, pt A, pt B, float r)
  {
    vec AB = V(A, B);
    pt M = P(0.5, A, 0.5, B);
    float x = n(AB) / 2;
  ...
  boolean isClockWise(vec U, vec V, vec W){
    float b1 = det2(U, V);
    float b2 = det2(U, W);
    float b3 = det2(V, W);
    if(b1 > 0){
...
//**** Function For roof
//************************************************************************
  float area(pt[] polyloop){
    float result = 0;
    pt O = P(0, 0, 0);
    for(int i = 0; i < polyloop.length; ++i){
      result += det2(V(O, polyloop[i]), V(O, polyloop[(i + 1) % polyloop.length]));
    }
    return result;
  }